...
def test_find_port_conflicts_detects_host_process(tmp_path: Path, monkeypatch) -> None:
    ...
    monkeypatch.setattr(diag_mod, "_find_host_listeners", lambda port: [
    ...
    conflicts = diag_mod._find_port_conflicts({18000: "core-runtime"}, plan)
...
def test_kill_process(monkeypatch) -> None:
    ...
    monkeypatch.setattr(diag_mod.os, "kill", fake_kill)
    assert diag_mod._kill_process(99, signal_name="term") is True
